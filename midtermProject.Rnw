...
Abstract: By using Boston University's Healthy Minds survey, I am looking at which group experiences the most discrimination. 
As expected, the data shows a higher percentage of people who are transgender experiencing discrimination. 
...
invisible(lapply(unique(key),pullout))#all columns are now grouped with based on the inital part of their column header
...
#cleaning up the race column by aggregating into 5 categories and cleaning up the text by manually going through each text entry 
...
#Each reponder is grouped into undergraduate, graduate and other. Academic Status was cleaned up based on the highest level of degree completed. Therefore if a responder is in a undergraduate and graduate program, he/she would count as a graduate student. 
@
...
#To make future analysis easier, we combinded 6 categories of gender into 4 categories.
@
...
#changing the binary data into categoryical data for easier communication and understanding. 
@
...
#with all of the relevant demographic data cleaned, the columns are now saved in R. 
@
...
#looking at specific variables that I'm interested in. Putting them into dataframes for easier EDA 
@
...
#Understand the central tendencies of the data, specifically the belong columns as well as the discrimination columns 
@
...
#looking at discrimination data and the degree in which the responders have experienced or witnessed discrimination 
#the new datafram of specifically discrimination data along with demographic data in saved into R. 
@

Another way of filtering the data
<<>>=
#load("dis.Rdata")
#dis_female<-mean(dis %>% filter(dis$gender=="female"))
#dis_male<-mean(dis %>% filter(dis$gender=="male"))
#dis_trans<-mean(dis %>% filter(dis$gender=="trans gender"))
#dis_other<-mean(dis %>% filter(dis$gender=="other"))
...
#ggplot(dis_avg,aes(x=gender_c,y=Dis_Gender))+geom_point(aes(size=3))+ggtitle("DiscrimAvg by Gender")
@
...
"dis.RData" is the cleaned data 

#worked with everyon in the BU Healthy Minds group 
